...
class ABB : public AB<Key> {
 public:
  ...
  bool SearchBinaryData(NodeB<Key>* nodo, const Key& data) const;
 ...
 */
template<typename Key>
bool ABB<Key>::SearchBinaryData(NodeB<Key>* node, const Key& data) const {
  bool result{false};
  // if (node != nullptr) {
  //   if (data == node->GetData()) result = true;
  //   else {
  //     result = this->SearchBinaryData(node->GetLeftSon(), data);
  //     if (!result) result = this->SearchBinaryData(node->GetRightSon(), data);
  //   }
  // }
  // return result;
  if (node == nullptr) return false;
  if (data == node->GetData()) return true;
  if (data < node->GetData()) return this->SearchBinaryData(node->GetLeftSonReferenced(), data );
  return this->SearchBinaryData(node->GetRightSonReferenced(), data );
  return result;
}
...
bool ABB<Key>::Search(const Key& data) const {
  bool result{false};
  result = this->SearchBinaryData(this->root_, data);
  return result;
}
#endif // _ABB_H_